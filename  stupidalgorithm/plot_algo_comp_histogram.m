...
func_cap={'sphere','griewank','rastrigin','ackey','f5','rosenbrock','step','quartic_with_noise','ws_location','f2','f8','camelback','f12','f13','f3','f4','f14','f15', ...
    'm_foxhole','m_langerman','m_michaelwicz','chebyshev'};
algo_cap={'PSO','mPSO','arPSO','dPSO','*dPSO','PSObc', ...
    'DE','bbDE','sDE','msDE','spDE','jDE', ...
    'fEP','ifEP', ...
    'dgEA', ...
    'eDE','dmDE','DE/EDA', ...
    'EDA', ...
...
for f_idx=1:func_num % for every tested functon
    ...
    if ( display_graph )
        ...
        plot_title=strcat(func_desc,'Very Best of All Run');
        h=figure;
        ...
        ylabel('Very Best of All Run');
        h = findobj(gca,'Type','patch');
        set(h,'EdgeColor','w');
        % save current figure
        plot_title=strcat(func_desc,'very_best_comparision.png');
        ...
        for i=1:algo_num % End Of File?
            ...
            l_p=line_property(z,:);
            plot(gen_bst_data(:,1),log(gen_bst_data(:,2)),l_p,'MarkerSize',6);
            hold on;
        ...
        for i=1:algo_num
            ...
            plot(gen_div_data(:,1),log(gen_div_data(:,2)),line_property(i,:),'MarkerSize',6);
            hold on;
            leg_txt_orig(i)=algo_cap(algo_code_list{i});
            leg_txt(i)=leg_txt_orig(i);
            if ( i>1 )
        ...
        for i=1:algo_num
            ...
            plot(gen_rad_data(:,1),log(gen_rad_data(:,2)),line_property(i,:),'MarkerSize',6);
            hold on;
            leg_txt_orig(i)=algo_cap(algo_code_list{i});
            leg_txt(i)=leg_txt_orig(i);
            if ( i>1 )
            ...
            while ~feof(fidin) % End Of File?
                ...
                if ~isempty(tline) % empty line?
                    ...
                    if ( cur_func==cur_func_code )
                        ...
                        plot(gen_vel_div_data(:,1),log(gen_vel_div_data(:,2)),line_property(i,:),'MarkerSize',6);
                        hold on;
        ...
        if ( has_de )
            ...
            for i=1:algo_num
                ...
                if (  algo_type_list{i}==NSDE_Algo_Code )
                    title_list_orig{i}=strcat(func_desc,'NSDE Algorithm');
                end
