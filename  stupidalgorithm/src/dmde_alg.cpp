...
namespace de
{
	namespace dmde
	{
		int dmde_alg::run()
		{
			...
			int shuffle_size=pop_size-2;
			vector<int> vec_idx(shuffle_size);
			...
			for ( m_cur_run=0;m_cur_run<max_run;m_cur_run++ )
			{
				...
				stat_eda_dist(pop);// dmde SPECIFIC
				...
				while ( false==(*m_pstop_cond) ) // for every iteration
				{
					...
					for ( i=0;i<pop_size;i++ )
					{
						...
						for ( j=0;j<num_dims;j++ )
						{
							...
							{
								double eda_x_dim=gen_eda_x(j);
								trial_pop[i].x[j]=0.5*(eda_x_dim+trial_pop[i].x[j]) +
									f_val*( eda_x_dim-trial_pop[i].x[j]+trial_pop[i2].x[j]-trial_pop[i3].x[j] );
								// boundaries check
