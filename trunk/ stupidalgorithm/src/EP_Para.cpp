...
namespace ep
{
	int ep_para::read_para(string conf_path)
	{
		...
		try
		{
			...
			parse_seq_var(m_ext_vm,"tour_size",tour_size);

			parse_seq_var(m_ext_vm,"mut_type",mut_type);

			parse_seq_var(m_ext_vm,"ini_eta",ini_eta);
