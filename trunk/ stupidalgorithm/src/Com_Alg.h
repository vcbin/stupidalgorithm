#ifndef STUPIDALGO_ALG_COMMON_FUNC
#define STUPIDALGO_ALG_COMMON_FUNC
...
protected:
	...
	void set_orig_pop(population &pop)
	{
		...
		{
			std::ofstream ini_pop_file(ini_pop_path.c_str());
			this->write_initial_pop(ini_pop_file,pop);
	...
	{
		std::ofstream avg_bst_file(m_com_out_path.avg_bst_path.c_str());
		this->print_gen_avg_val(avg_bst_file,m_alg_stat.gen_avg_bst_val);
	}

	// record average diversity of all runs in every generation
	void write_avg_diver_per_gen()
	{
		std::ofstream avg_div_file(m_com_out_path.avg_div_path.c_str());
		this->print_gen_avg_val(avg_div_file,m_alg_stat.gen_avg_div_val);
	}

	// record average search radius of all runs in every generation
	void write_avg_rad_per_gen()
	{
		std::ofstream avg_rad_file(m_com_out_path.avg_rad_path.c_str());
		this->print_gen_avg_val(avg_rad_file,m_alg_stat.gen_avg_rad_val);
	}

	void write_all_bst_val()
	{
		std::ofstream all_bst_val_file(m_com_out_path.all_bst_val_path.c_str());
		this->print_all_gbest_val(all_bst_val_file,m_alg_stat.run_gbest_val);
	}
	...
	public:
		stop_gen(com_alg *pself)
			:stop_cond(pself) {}
		operator bool() { return stop_cond::self->m_cur_gen >= (stop_cond::self->m_ppara->get_max_gen()); }
	};
	...
	public:
		stop_eval(com_alg *pself)
			:stop_cond(pself) {}
		operator bool() { return stop_cond::self->m_alg_stat.eval_num >= (stop_cond::self->m_ppara->get_stop_val()); }
	};
	...
	public:
		stop_stag(com_alg *pself)
			:stop_cond(pself) {}
		operator bool() { return stop_cond::self->m_alg_stat.num_stag >= (stop_cond::self->m_ppara->get_stop_val()); }
	};
	...
	void alloc_stop_cond()
	{
		...
		{
		case stoptype_eval:
			m_pstop_cond=boost::shared_ptr<stop_cond>(new stop_eval(this));
			break;
		case stoptype_stag:
			m_pstop_cond=boost::shared_ptr<stop_cond>(new stop_stag(this));
			break;
		default:
			m_pstop_cond=boost::shared_ptr<stop_cond>(new stop_gen(this));
			break;
		};
	}
