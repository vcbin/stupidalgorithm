...
#include <map>
#include <strstream> // operator<< definition
#include <iostream>
...
public:
	...
	inline T get_seq_val(const std::string& para) 
	{
		...
		else // target type and parsed type doesn't match,try to correct it
		{
			...
			str_err<<"\n"
				...
				<<"."
				<<"\n";
			throw std::bad_cast(str_err.str().c_str());
		}
		return ret_val; 
	}// end function get_seq_val

	template<>
	inline int get_seq_val<int>(const std::string& para) 
	{
		int ret_val;
		boost::any any_val=get_cur_seq_val(para);
	...
	template<>
	inline double get_seq_val<double>(const std::string& para) 
	{
		double ret_val;
		boost::any any_val=get_cur_seq_val(para);
